...
internal sealed class IndentedTextWriter : IDisposable
{
    ...
    {
        this._builder = new ImmutableArrayBuilder<char>();
        this._currentIndentationLevel = 0;
        this._currentIndentation = "";
        this._availableIndentations = new string[4];
        this._availableIndentations[0] = "";

        for (int i = 1, n = this._availableIndentations.Length; i < n; i++)
        {
            this._availableIndentations[i] = this._availableIndentations[i - 1] + DefaultIndentation;
        }
    }
    ...
    public Span<char> Advance(int requestedSize)
    {
        ...
        if (this._builder.Count == 0 || this._builder.WrittenSpan[^1] == DefaultNewLine)
        {
            this._builder.AddRange(this._currentIndentation.AsSpan());
        }

        return this._builder.Advance(requestedSize);
    }
    ...
    public void IncreaseIndent()
    {
        ...
        if (this._currentIndentationLevel == this._availableIndentations.Length)
        {
            Array.Resize(ref this._availableIndentations, this._availableIndentations.Length * 2);
        }
        ...
        this._currentIndentation = this._availableIndentations[this._currentIndentationLevel]
            // ReSharper disable once NullCoalescingConditionIsAlwaysNotNullAccordingToAPIContract
            ??= this._availableIndentations[this._currentIndentationLevel - 1] + DefaultIndentation;
    }
    ...
    public void WriteIf(
        ...
        ref WriteIfInterpolatedStringHandler handler)
    {
        _ = this;
    }
    ...
    {
        if (skipIfPresent && this._builder.WrittenSpan is [.., '\n', '\n'])
        {
            return;
        }

        this._builder.Add(DefaultNewLine);
    }
    ...
    {
        return this._builder.WrittenSpan.Trim().ToString();
    }
    ...
    private void WriteRawText(ReadOnlySpan<char> content)
    {
        ...
        this._builder.AddRange(content);
    }
    ...
    public delegate void Callback<in T>(T value, IndentedTextWriter writer);
    ...
    public readonly ref struct WriteInterpolatedStringHandler
    {
        ...
        public void AppendFormatted<T>(T value, string? format)
        {
            ...
            {
                this._writer.Write(formattable.ToString(format, CultureInfo.InvariantCulture));
            }
    ...
    public readonly ref struct WriteIfInterpolatedStringHandler
    {
            ...
            out bool shouldAppend)
        {
            if (condition)
            {
                this._handler = new WriteInterpolatedStringHandler(
                    literalLength,
                    formattedCount,
                    writer);
        ...
        public void AppendFormatted<T>(T value, string? format)
        {
            this._handler.AppendFormatted(value, format);
        }
    }
}
