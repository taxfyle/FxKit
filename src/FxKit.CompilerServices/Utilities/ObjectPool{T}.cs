    ...
    where T : class
{
    ...
    public T Allocate()
    {
        var item = this._firstItem;

        if (item is null || item != Interlocked.CompareExchange(ref this._firstItem, value: null, item))
        {
            item = AllocateSlow();
        }
    ...
    {
        foreach (ref var element in this._items.AsSpan())
        {
            var instance = element._value;

            if (instance is not null)
            {
                if (instance == Interlocked.CompareExchange(ref element._value, value: null, instance))
                {
                    return instance;
                }
            }
